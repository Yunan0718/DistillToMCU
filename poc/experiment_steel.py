"""
DistillToMCU — 钢铁工业能耗实验 (UCI 851)
==========================================
工业能源控制：LLM 根据能耗/功率因数/CO2/负载等级决定是否节能。

Usage:
    python experiment_steel.py --seed 42 --days 30
"""
...
SYSTEM_PROMPT = """You are an industrial energy controller for a steel plant.
You receive machine power readings. Decide if any device action is needed.

Available devices:
- fan: cooling fan, speed (1-3)
- led: energy-saving mode indicator, brightness (0-100)
- curtain: load isolation, position (0-100, 0=isolated, 100=connected)

Rules:
- If energy usage is high, turn on the fan for cooling
- If power factor is low, enable energy-saving mode (led)
- If CO2 emissions are high, reduce load (curtain)
...
class SteelExecutor:
    FIELDS = ["usage_kwh", "lagging_power", "power_factor", "co2", "load_level"]
    UNITS = {"usage_kwh": "kWh", "lagging_power": "kVarh",
             "power_factor": "%", "co2": "t", "load_level": ""}
    ...
    def handle(self, sensors):
        ...
        query = "Current machine readings:\n" + "\n".join(sensor_lines) + \
...
def main():
    ...
    with open(os.path.join(os.path.dirname(__file__), 'data', 'steel',
                           'snapshots.json')) as f:
        raw = json.load(f)
    ...
    odir = args.output_dir or os.path.join(
        os.path.dirname(__file__), 'output', f'steel_seed{args.seed}')
    os.makedirs(odir, exist_ok=True)
    ts = TraceStore(output_dir=odir)
    re = RuleEngine()
    di = Distiller(re, llm_client=llm_client)
    ex = SteelExecutor(re, ts)

    print('STEEL industrial energy -> LLM')
