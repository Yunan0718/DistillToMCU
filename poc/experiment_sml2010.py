"""
DistillToMCU — SML2010 室内气候实验
===================================
传感器读数直接发给 LLM，LLM 自行决定是否控制设备（与 UCI 同一范式，
但 SML2010 无运动/占用传感器，motion 诚实空缺）。

Usage:
    python experiment_sml2010.py --seed 42 --days 30
"""
...
You receive indoor climate sensor readings. Decide if any device action is needed.
...
- If it is too bright, close the curtain
- If the temperature is rising, turn on the fan
...
class SML2010Executor:
    ...
    def handle(self, sensors):
        ...
        sensor_lines = []
        for k in ['temperature', 'humidity', 'light', 'co2']:
            v = sensors.get(k)
            if v is not None:
                unit = ('C' if k == 'temperature' else '%' if k == 'humidity'
                        else 'lux' if k == 'light' else 'ppm')
...
def main():
    ...
    with open(os.path.join(os.path.dirname(__file__), 'data', 'sml2010',
                           'snapshots.json')) as f:
        raw = json.load(f)
    ...
    odir = args.output_dir or os.path.join(
        os.path.dirname(__file__), 'output', f'sml2010_seed{args.seed}')
    os.makedirs(odir, exist_ok=True)
    ts = TraceStore(output_dir=odir)
    re = RuleEngine()
    di = Distiller(re, llm_client=llm_client)
    ex = SML2010Executor(re, ts)

    print('SML2010 real indoor climate -> LLM (no motion sensor)')
